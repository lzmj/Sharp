...
namespace SharpTools
{
    public class DBInfo
    {
        ...
        private Dictionary<string, DateTime> GetTableStruct_Modify(out bool isAllMyISAM, out List<string> lstTabNames)
        {
            ...
            if (Db.DBaseType == DataBaseType.MsSql)
            {
            ...
            else if (Db.DBaseType == DataBaseType.Oracle)
            {
            ...
            else if (Db.DBaseType == DataBaseType.MySql)
            {
            ...
            else if (Db.DBaseType == DataBaseType.Sqlite)
            {
                selSql = "SELECT name FROM sqlite_master WHERE type='table' order by name";
                lstTabNames = Db.QueryTable(selSql).GetFirstCol<string>();
            }
            lstTabNames = lstTabNames.OrderBy(t => t).ToList();
            return dict_SD;
        }
        ...
        public bool UpsertTableComment(string tableName, string comment)
        {
            ...
            switch (Db.DBaseType)
            {
                ...
                case DataBaseType.Oracle:
                    ...
                    break;
                case DataBaseType.Sqlite://Sqlite 不支持 表、列 注释说明
                    return false;
                default:
        ...
        public string GetColumnComment(string tableName, string columnName)
        {
            ...
            {
                case DataBaseType.MySql:
                    strSql = "SELECT COLUMN_COMMENT FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = '" + tableName + "' AND table_schema = '" + Db.DBConn.Database + "' AND column_name = '" + columnName + "'";
                    break;
                case DataBaseType.MsSql:
        ...
        {
            Dictionary<string, string> dict_result = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
            string strSql = string.Empty;
            switch (Db.DBaseType)
            {
                case DataBaseType.MySql:
...
WHERE a.xtype='U' AND a.name <>'sysdiagrams' AND a.name <>'dtproperties' ORDER BY a.name asc";
            ...
            //过滤 Oracle 中表名有小写的情况            
            if (Db.DBaseType == DataBaseType.Oracle)
            {
                dict = dict.Where(t => Regex.IsMatch(t.Key, "^[A-Z_$]+$")).ToDictionary(k => k.Key, v => v.Value);
            }
            else if (Db.DBaseType == DataBaseType.MySql)
            {
                //MySql的表 存储引擎如果是InnoDB，则表 TABLE_COMMENT会是 “流程状态表; InnoDB free: 96256 kB” 这种
        ...
        public List<ColumnInfo> GetAllColumnInfo(string tableName)
        {
            ...
            switch (Db.DBaseType)
            {
                case DataBaseType.MySql:
                    ...
                    break;
                case DataBaseType.MsSql:
                    strSql = @"SELECT a.colorder Colorder,a.name ColumnName,b.name TypeName,(case when (SELECT count(*) FROM sysobjects  WHERE (name in (SELECT name FROM sysindexes  WHERE (id = a.id) AND (indid in  (SELECT indid FROM sysindexkeys  WHERE (id = a.id) AND (colid in  (SELECT colid FROM syscolumns WHERE (id = a.id) AND (name = a.name)))))))  AND (xtype = 'PK'))>0 then 1 else null end) IsPK,(case when COLUMNPROPERTY( a.id,a.name,'IsIdentity')=1 then 1 else null end) IsIdentity,  CASE WHEN (charindex('int',b.name)>0) OR (charindex('time',b.name)>0) THEN NULL ELSE  COLUMNPROPERTY(a.id,a.name,'PRECISION') end as [Length], CASE WHEN ((charindex('int',b.name)>0) OR (charindex('time',b.name)>0)) THEN NULL ELSE isnull(COLUMNPROPERTY(a.id,a.name,'Scale'),null) end as Scale,(case when a.isnullable=1 then 1 else null end) CanNull,isnull(e.text,'') DefaultVal,isnull(g.[value], ' ') AS DeText FROM  syscolumns a left join systypes b on a.xtype=b.xusertype inner join sysobjects d on a.id=d.id and d.xtype='U' and d.name<>'dtproperties' left join syscomments e on a.cdefault=e.id left join sys.extended_properties g on a.id=g.major_id AND a.colid=g.minor_id left join sys.extended_properties f on d.id=f.class and f.minor_id=0 where b.name is not NULL and d.name=N'" + tableName + "' order by a.id,a.colorder";
